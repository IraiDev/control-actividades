...
const Todo = () => {
  ...
  return (
    <>
      <header className='flex items-center justify-between bg-white p-4 m-2 rounded-lg border shadow-lg'>
        <div className='flex items-center gap-2'>
          <span className='flex items-baseline gap-2 text-lg font-semibold text-gray-800'>
            <i className={icon_list + ' hidden sm:block'} />
            <p className='max-w-[130px] sm:max-w-full truncate'>{title_list}</p>
          </span>
          <label htmlFor='importance'>
            <input
              ...
              onChange={e => {
                e.target.checked
                  ? setImportant({ select: e.target.checked, value: 'high' })
      ...
      >
        {tasks.length > 0 ? (
          tasks.map(task => {
            let condition
            if (important.select)
              condition = task.importance === important.value
            else condition = true
            if (condition) {
              return (
                <TodoCard
                  ...
                  deleteTodo={() =>
                    handleDeleteTodo({ idTodo: task.id, title: task.title })
                  }
                  {...task}
                />
              )
