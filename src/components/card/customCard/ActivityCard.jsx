...
const ActivityCard = (props) => {
  ...
  const { optionsArray, user, filters, saveFilters } =
  ...
  const openModalClone = () => {
    ...
    setActOptions({
      ...
      sp:
        data.id_sub_proy !== 0
          ? optionsArray.subProjects.find(
              (item) => item.value === data.id_sub_proy
            )
          : actOptions.sp,
      ur:
        data.id_revisor !== 0
  ...
  const validation = () => {
    ...
    const vPriority =
      prio_act?.toString().trim() === "" || Number(prio_act) <= 0;
    ...
    const vRdisE =
      actOptions.ta?.value === 1
        ? actOptions.ur?.id === actOptions.ue?.id
        : false;
      ...
      >
        ...
        <CardFooter>
          ...
          >
            <i
              className={
                ESTADO_play ? "fas fa-pause fa-sm" : "fas fa-play fa-sm"
              }
            />
          </Button>
