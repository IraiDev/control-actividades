...
const Revision = () => {
   ...
   const [{ id, title, desc, ticket, reject_gloss }, onChangeValues, reset, onPreset] =
      useForm({
         id: '',
         title: '',
         desc: '',
   ...
   const onFilter = () => {
      const filters = {
         estado:
            options.st?.length > 0 && options.st.some((o) => o.value !== null)
               ? options.st.map((item) => item.value)
               : [],
         proyecto:
            options.pr?.length > 0 && options.pr.some((o) => o.value !== null)
               ? options.pr.map((item) => item.value)
               : [],
         encargado:
            options.ue?.length > 0 && options.ue.some((o) => o.value !== null)
               ? options.ue.map((item) => item.label)
               : [],
         solicitante:
            options.us?.length > 0 && options.us.some((o) => o.value !== null)
               ? options.us.map((item) => item.label)
               : [],
         subProy:
            options.sp?.length > 0 && options.sp.some((o) => o.value !== null)
               ? options.sp.map((item) => item.value)
               : [],
         revisor:
            options.ur?.length > 0 && options.ur.some((o) => o.value !== null)
               ? options.ur.map((item) => item.id)
               : [],
         id_tipo_actividad:
            options.ita?.length > 0 && options.ita.some((o) => o.value !== null)
               ? options.ita.map((item) => item.value)
               : [],
         id_actividad: id,
         titulo: title,
   ...
   // const onChangeCheckedActivity = async ({
   //   id,
   //   title,
   //   estado,
   //   revisado,
   ...
   //         title: "Atención",
   //         content: `${
   //           revisado ? "Aprobar" : "Rechazar"
   //         } revisión de la siguiente actividad: <strong>${title}</strong>, <strong>${id}</strong>`,
   ...
   const onChangeCheckedActivity = async ({ id, title, estado, revisado }) => {
      Alert({
         title: 'Atención',
         content: `${
            revisado ? 'Aprobar' : 'Rechazar'
         ...
         action: async () => {
            revisado
               ? await toggleCheckActivity({ id_actividad: id, estado, revisado })
               : openModalReject(id, estado)
         },
      })
   }
   ...
   const handleToggleRejectActivity = () => {
      toggleCheckActivity({
         id_actividad: activityData.id,
         estado: activityData.estado,
         revisado: false,
         glosa_rechazo: reject_gloss,
      })
      onCloseModal()
   }

   const onPlay = ({ id_actividad, tipo_pausa }) => {
      onPlayPause({ id_actividad, tipo_pausa })
   }
   ...
   useEffect(() => {
      setOptions({
         st: !options?.st?.some((o) => o.value === null)
            ? optionsArray?.status?.filter((os) => {
                 return prFilters.estado.includes(os.value)
              })
            : [{ value: null, label: 'Todos' }],

         pr: !options?.pr?.some((o) => o.value === null)
            ? optionsArray?.projects?.filter((op) => {
                 return prFilters.proyecto.includes(op.value)
              })
            : [{ value: null, label: 'Todos' }],

         ue: !options?.ue?.some((o) => o.value === null)
            ? optionsArray?.users?.filter((ou) => {
                 return prFilters.encargado.includes(ou.value)
              })
            : [{ value: null, label: 'Todos' }],

         us: !options?.us?.some((o) => o.value === null)
            ? optionsArray?.users?.filter((ou) => {
                 return prFilters.solicitante.includes(ou.value)
              })
            : [{ value: null, label: 'Todos' }],

         ur: !options?.ur?.some((o) => o.value === null)
            ? optionsArray?.users?.filter((ou) => {
                 return prFilters.revisor.includes(ou.id)
              })
            : [{ value: null, label: 'Todos' }],

         sp: !options?.sp?.some((o) => o.value === null)
            ? optionsArray?.subProjects?.filter((os) => {
                 return prFilters.subProy.includes(os.value)
              })
            : [{ value: null, label: 'Todos' }],

         ita: !options?.ita?.some((o) => o.value === null)
            ? optionsArray?.activity_type?.filter((oi) => {
      ...
      <>
               ...
               <THead>
                  <tr className="text-center capitalize">
                     <Th>
                        <label
                           htmlFor="solo_padres"
                           className="flex items-center gap-2 px-2 py-1 mx-auto mt-2 text-sm font-semibold rounded-full cursor-pointer w-max bg-zinc-200"
                        >
                           Solo padres
                           <input
                     ...
                     <Th>
                        <SelectFilter
                           ...
                           filterDown={() => setPROrder({ orden_tipo_actividad: 'desc' })}
                           filterUp={() => setPROrder({ orden_tipo_actividad: 'asc' })}
                           upActive={setActive({
                     ...
                     <Th>
                        <SelectFilter
                           ...
                           filterDown={() => setPROrder({ orden_proyecto: 'desc' })}
                           filterUp={() => setPROrder({ orden_proyecto: 'asc' })}
                           upActive={setActive({
                     ...
                     <Th>
                        <InputFilter
                           ...
                           value={title}
                           onChange={onChangeValues}
                           filterDown={() => setPROrder({ orden_actividad: 'desc' })}
                           filterUp={() => setPROrder({ orden_actividad: 'asc' })}
                           upActive={setActive({
                     ...
                     <Th>
                        <SelectFilter
                           ...
                           filterDown={() => setPROrder({ orden_estado: 'desc' })}
                           filterUp={() => setPROrder({ orden_estado: 'asc' })}
                           upActive={setActive({
                              param: 'orden_estado',
                              value: 'asc',
                           })}
                     ...
                     <Th>
                        <SelectFilter
                           ...
                           filterDown={() => setPROrder({ orden_solicitante: 'desc' })}
                           filterUp={() => setPROrder({ orden_solicitante: 'asc' })}
                           upActive={setActive({
                     ...
                     <Th>
                        <SelectFilter
                           ...
                           filterDown={() => setPROrder({ orden_encargado: 'desc' })}
                           filterUp={() => setPROrder({ orden_encargado: 'asc' })}
                           upActive={setActive({
                     ...
                     <Th>
                        <SelectFilter
                           ...
                           filterDown={() => setPROrder({ orden_revisor: 'desc' })}
                           filterUp={() => setPROrder({ orden_revisor: 'asc' })}
                           upActive={setActive({
                              param: 'orden_revisor',
                              value: 'asc',
                           })}
                  ...
                  <tr className="text-center capitalize">
                     ...
                     <Th primary>
                        <div className="w-full" title="Tiempo cobrable (T.cliente)">
                           Tiempo (hrs)
                        </div>
                     </Th>
                     <Th primary>
                        <div className="flex items-baseline justify-center gap-2">
                           ...
                           >
                              <i
                                 className={
                                    multiline ? 'fas fa-angle-up' : 'fas fa-angle-down'
                                 }
                              />
                           </Button>
                        </div>
                     ...
                     <Th primary>
                        <span title="Fecha y hora de paso a revision">fecha P.R.</span>
                     </Th>
                     <Th primary isStickyRight>
                     ...
                     activitiesPR.map((act, i) => (
                        <tr
                           ...
                           className={`
                              ...
                              ${
                                 activityRunning.id === act.id_det_padre &&
                        ...
                        >
                           <Td
                              onDoubleClick={() =>
                                 navigate(
                                    `actividad-pr-detalle/${act.id_det}?type_detail=pr`
                                 )
                              }
                           >
                           ...
                           >
                              <span
                                 className={`
                                    ...
                                    ${
                                       act.id_tipo_actividad === 1
                                          ? 'bg-indigo-200 text-indigo-500'
                                          : act.id_tipo_actividad === 2
                                          ? 'bg-emerald-200 text-emerald-500'
                                          : act.id_tipo_actividad === 3
                                          ? 'bg-red-200 text-red-500'
                                          : act.id_tipo_actividad === 4
                           ...
                           >
                              <span
                                 title={`(T. zionit) + (T. cliente) = (T. total) --> ${
                                    act?.tiempo_zionit
                                 } + ${act?.tiempo_cliente} = ${
                                    act?.tiempo_zionit + act?.tiempo_cliente
                                 }`}
                           ...
                           >
                              {moment(
                                 `${act.fecha_revicion} ${act.hora_revicion}`
                              ).format('DD-MM-yyyy, HH:MM') ?? 'sin fecha'}
                           </Td>

                           <TdSwitch
                              state={act.estado}
                              onDistribution={({ distribuciones, id_actividad }) =>
                                 onDistribution({ distribuciones, id_actividad })
                              }
                              getId={(id_callback) => setIsActive(id_callback)}
                              onChangeCheckedActivity={({
                                 id,
                                 title,
                                 revisado,
                                 estado,
                              ...
                              onPlay={() =>
                                 onPlay({
                                    ...
                                    tipo_pausa: act.estado === 3 ? 2 : 3,
                                 })
                              }
                              onPause={({ mensaje, tipo_pausa }) =>
                                 onPause({
                                    mensaje,
                                    tipo_pausa,
                                    id_actividad: act.id_det,
         ...
         >
            ...
            <footer className="flex justify-between mt-7">
               ...
               <Button
                  className="text-yellow-500 bg-yellow-100 hover:bg-yellow-200"
